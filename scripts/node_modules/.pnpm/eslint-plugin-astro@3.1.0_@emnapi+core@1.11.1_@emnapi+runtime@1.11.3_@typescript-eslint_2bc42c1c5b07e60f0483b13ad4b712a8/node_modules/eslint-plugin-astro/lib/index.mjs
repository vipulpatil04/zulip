...
function extractClassList(node, context) {
	if (node.value?.type === AST_NODE_TYPES.Literal) return { classList: [String(node.value.value)] };
...
function* extractClassListFromExpression(node, context) {
	if (node.type === AST_NODE_TYPES.ArrayExpression) {
		for (const element of node.elements) {
			...
			if (element.type === AST_NODE_TYPES.SpreadElement) yield* extractClassListFromExpression(element.argument, context);
			else yield* extractClassListFromExpression(element, context);
		}
		return;
	}
	if (node.type === AST_NODE_TYPES.ObjectExpression) {
		for (const prop of node.properties) if (prop.type === AST_NODE_TYPES.SpreadElement) yield* extractClassListFromExpression(prop.argument, context);
		else if (!prop.computed) if (prop.key.type === AST_NODE_TYPES.Literal) yield String(prop.key.value);
		else yield prop.key.name;
		else yield* extractClassListFromExpression(prop.key, context);
		return;
	}
	const sourceCode = context.sourceCode;
	const staticValue = getStaticValue(node, sourceCode.scopeManager.globalScope);
	if (staticValue) {
...
var no_unused_define_vars_in_style_default = createRule("no-unused-define-vars-in-style", {
	...
	create(context) {
		...
		return { "JSXElement > JSXOpeningElement[name.type='JSXIdentifier'][name.name='style']"(node) {
			...
			if (!defineVars.value || defineVars.value.type !== AST_NODE_TYPES.JSXExpressionContainer || defineVars.value.expression.type !== AST_NODE_TYPES.ObjectExpression) return;
			...
			const definedVars = defineVars.value.expression.properties.filter((prop) => prop.type === AST_NODE_TYPES.Property).map((prop) => ({
				prop,
	...
	create(context) {
		...
		class NewObjectProps {
			...
			fixObject({ fixer, object }) {
				...
				let text;
				if (isCommaToken(maybeComma)) text = this.toPropsString();
		...
		*/
		...
		function verifyAttr(attr) {
			...
			if (!attr.value || attr.value.type !== AST_NODE_TYPES.JSXExpressionContainer || attr.value.expression.type === AST_NODE_TYPES.JSXEmptyExpression) return;
				...
				*fixExpression(data) {
					if (element.array) {
						...
						if (object) {
							...
							const tokens = getParenthesizedTokens(element.node, sourceCode);
							const maybeComma = sourceCode.getTokenAfter(tokens.right);
							if (isCommaToken(maybeComma)) yield data.fixer.removeRange(maybeComma.range);
							else {
								const maybeBeforeComma = sourceCode.getTokenBefore(tokens.left);
								if (isCommaToken(maybeBeforeComma)) yield data.fixer.removeRange(maybeBeforeComma.range);
							}
							yield data.newProps.fixObject({
								fixer: data.fixer,
								object
			...
			*/
			function findClosestObject(array, target) {
				...
				for (let index = 0; index < length; index++) {
					...
					if (after?.type === AST_NODE_TYPES.ObjectExpression) return after;
					const before = beforeElements[index];
					if (before?.type === AST_NODE_TYPES.ObjectExpression) return before;
				}
				return null;
			}
			/** Visit expression */
			function visitElementExpression(node, state) {
				if (node.type === AST_NODE_TYPES.ConditionalExpression) verifyConditionalExpression(node, state);
					...
					if (isStringCallExpression(node) && node.arguments[0] && node.arguments[0].type !== AST_NODE_TYPES.SpreadElement) visitElementExpression(node.arguments[0], {
	...
	create(context) {
		...
		function verifyAttr(attr) {
			...
			verifyExpression(expression, function* (fixer) {
				if (expression.type === AST_NODE_TYPES.ArrayExpression) return;
		...
		function verifyExpression(node, transformArray, call) {
			if (node.type === AST_NODE_TYPES.TemplateLiteral) {
			...
			} else if (node.type === AST_NODE_TYPES.BinaryExpression) verifyBinaryExpression(node, transformArray);
				...
				if (node.callee.type === AST_NODE_TYPES.MemberExpression && getPropertyName(node.callee) === "trim") verifyExpression(node.callee.object, transformArray, ".trim()");
			}
		}
		...
		function verifyClsx(clsxCall) {
			if (clsxCall.node.type !== AST_NODE_TYPES.CallExpression) return;
			const callNode = clsxCall.node;
			const parent = callNode.parent;
			if (!parent || parent.type !== AST_NODE_TYPES.JSXExpressionContainer || parent.expression !== callNode) return;
			const parentParent = parent.parent;
			if (!parentParent || parentParent.type !== AST_NODE_TYPES.JSXAttribute || parentParent.value !== parent || getAttributeName(parentParent) !== "class:list") return;
			context.report({
		...
		return {
			Program(node) {
				...
				for (const call of referenceTracker.iterateEsmReferences({ clsx: { [ReferenceTracker.CALL]: true } })) verifyClsx(call);
			},
			JSXAttribute: verifyAttr,
			AstroTemplateLiteralAttribute: verifyAttr
		};
...
function isStringType(node) {
	...
	else if (node.type === AST_NODE_TYPES.TemplateLiteral) return true;
	else if (node.type === AST_NODE_TYPES.BinaryExpression) return isStringType(node.left) || isStringType(node.right);
...
function buildFlatConfigs() {
	...
	for (const configName of a11yConfigKeys) Object.defineProperty(configs, `jsx-a11y-${configName}`, {
		enumerable: true,
		get() {
			const baseRules = (getPluginJsxA11y()?.configs?.[configName] ?? {}).rules ?? {};
			const newRules = {};
			for (const ruleName of Object.keys(baseRules)) newRules[`astro/${ruleName}`] = baseRules[ruleName];
			return [...base_default, { rules: newRules }];
		}
	});
	return configs;
}
...
function getEspree() {
	...
	if (!espreeCache) espreeCache = espree;
	return espreeCache;
}
//#endregion
...
const all = {};
for (const rule of rules$1.filter((rule) => rule.meta.docs.available() && !rule.meta.deprecated)) all[rule.meta.docs.ruleId] = "error";
var all_default = [...recommended_default, { rules: {
	...all,
	...recommended_default[recommended_default.length - 1].rules
} }];
//#endregion
//#region src/esm-config-builder.ts
/**
...
const plugin = Object.assign(getPlugin(), { configs });
const meta = plugin.meta;
const rules = plugin.rules;
const processors = plugin.processors;
const environments = plugin.environments;
//#endregion
export { configs, plugin as default, environments, meta, processors, rules };
