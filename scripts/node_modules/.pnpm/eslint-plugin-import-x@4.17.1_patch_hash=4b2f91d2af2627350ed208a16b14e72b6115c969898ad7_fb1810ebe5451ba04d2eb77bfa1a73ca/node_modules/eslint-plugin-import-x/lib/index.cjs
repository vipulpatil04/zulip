	...
	static parse(filepath, content, context) {
		...
		for (const n of ast.body) {
			...
			if (n.type === "TSExportAssignment") {
				...
				const exportedName = "expression" in n && n.expression && ("name" in n.expression && n.expression.name || "id" in n.expression && n.expression.id && n.expression.id.name) || null;
				const getRoot = (node) => {
...
var extensions_default = createRule({
	...
	create(context) {
		...
		return moduleVisitor((source, node) => {
			...
			const extension = node_path.default.extname(resolvedToDts ? importPath : resolvedPath || importPath).slice(1);
			const isPackage = isExternalModule(importPath, resolve(importPath, context), context) || isScoped(importPath);
			if (!extension || !importPath.endsWith(`.${extension}`)) {
				if (resolvedToDts && !extension && isPackage) return;
			...
			"Program:exit"() {
				for (const [objectName, lookups] of allPropertyLookups.entries()) {
					...
					for (const { propName, node } of lookups) {
						...
						if (fileImport.exportMap.hasExportAssignment) continue;
	...
	create(context) {
		function createCheckDefault(nameKey) {
			return function checkDefault(defaultSpecifier) {
				...
				if (exportMapOfImported.hasExportAssignment) return;
