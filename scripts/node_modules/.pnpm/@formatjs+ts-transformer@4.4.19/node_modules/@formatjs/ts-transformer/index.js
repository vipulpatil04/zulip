...
//#endregion
//#region packages/ts-transformer/normalize-whitespace.ts
...
function extractMessageDescriptor(ts, node, { overrideIdFn, extractSourceLocation, preserveWhitespace, flatten, throws = true, onMsgError }, sf) {
		...
		if (name && ts.isIdentifier(name) && initializer) {
			...
			if (ts.isStringLiteral(value)) switch (name.text) {
				...
				case "defaultMessage":
					...
					break;
				case "description": msg.description = value.text;
			}
				...
				switch (name.text) {
					...
					case "defaultMessage":
						...
						break;
					case "description": msg.description = template.text;
				}
				...
				if (ts.isStringLiteral(expression)) switch (name.text) {
					...
					case "defaultMessage":
						...
						break;
					case "description": msg.description = expression.text;
				}
					...
					if (isStatic) switch (name.text) {
						...
						case "defaultMessage":
							...
							break;
						case "description": msg.description = result;
					}
	...
	if (msg.defaultMessage && overrideIdFn) switch (typeof overrideIdFn) {
		case "string":
			...
			break;
		case "function": msg.id = overrideIdFn(msg.id, msg.defaultMessage, msg.description, sf.fileName);
	}
...
//#endregion
export { interpolateName, normalizeMessageWhitespace, transform, transformWithTs };
