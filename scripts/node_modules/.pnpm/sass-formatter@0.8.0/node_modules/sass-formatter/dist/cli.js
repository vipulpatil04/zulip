 ...
 */
function validateAndMergeConfig(parsed, defaults, configPath) {
    ...
    for (const key of Object.keys(defaults)) {
        ...
        if (Object.prototype.hasOwnProperty.call(parsed, key)) {
            ...
            if (key === 'lineEnding') {
                if (val !== 'LF' && val !== 'CRLF') {
                    errors.push(`Invalid value for "lineEnding": ${JSON.stringify(val)} (allowed: "LF" | "CRLF")`);
                }
                else {
                    merged[key] = val;
                }
            ...
            if (defaultType !== valType) {
                ...
                errors.push(`Invalid type for "${key}": expected ${defaultType}, got ${valType}`);
            }
            else {
                merged[key] = val;
            }
...
function loadConfig(configPath) {
    const finalPath = configPath
        ...
        : path_1.default.resolve(process.cwd(), '.sassformatterrc.json');
    if (!fs_1.default.existsSync(finalPath)) {
        // No config file in cwd -> use defaults
        return index_1.defaultSassFormatterConfig;
    }
    // Read file and parse JSON
    let raw;
    try {
        raw = fs_1.default.readFileSync(finalPath, 'utf-8');
    }
    catch (err) {
        console.error(`Failed to read config file: ${finalPath}\n${err.message}`);
        process.exit(1);
    }
    let parsed;
    try {
        ...
        console.error(`Failed to parse JSON in config file: ${finalPath}\n${err.message}`);
        process.exit(1);
    }
    // Validate & merge
    try {
        const merged = validateAndMergeConfig(parsed, index_1.defaultSassFormatterConfig, finalPath);
        return merged;
    }
    catch (err) {
...
`);
        console.log(JSON.stringify(index_1.defaultSassFormatterConfig, null, 2));
        process.exit(0);
    }
    ...
    for (const file of files) {
        ...
        let input;
        try {
            input = fs_1.default.readFileSync(filePath, 'utf-8');
        }
        catch (err) {
        ...
        try {
            ...
            const maybe = index_1.SassFormatter.Format(input, config);
            formatted = typeof maybe === 'string' ? maybe : await maybe;
        }
        catch (err) {
            console.error(`Error formatting file: ${file}\n${err.message}`);
            exitCode = 1;
            continue;
        }
        if (check) {
            ...
            try {
                ...
                fs_1.default.writeFileSync(filePath, formatted, 'utf-8');
