...
var nesting__default = /*#__PURE__*/_interopDefaultLegacy(nesting);

const functionalSelectorMatch = /(^|[^\w-])(%[_a-zA-Z]+[_a-zA-Z0-9-]*)([^\w-]|$)/i; // plugin

const postcssExtendRule = rawopts => {
  // options ( onFunctionalSelector, onRecursiveExtend, onUnusedExtend)
  const opts = Object(rawopts);
  const extendMatch = opts.name instanceof RegExp ? opts.name : 'name' in opts ? new RegExp(`^${opts.name}$`, 'i') : 'extend';
  return {
    ...
    OnceExit(root, {
      ...
      const extendedAtRules = new WeakMap(); // for each extend at-rule
      ...
      root.walkAtRules(extendMatch, extendAtRule => {
        ...
        if (!extendedAtRules.has(extendAtRule)) {
          extendedAtRules.set(extendAtRule, true); // selector identifier

          const selectorIdMatch = getSelectorIdMatch(extendAtRule.params, postcss); // extending rules

          const extendingRules = getExtendingRules(selectorIdMatch, extendAtRule); // if there are extending rules

          if (extendingRules.length) {
            // replace the extend at-rule with the extending rules
            extendAtRule.replaceWith(extendingRules); // transform any nesting at-rules

            const cloneRoot = postcss.root().append(parent.clone()); // apply nesting (sync)

            postcss([nesting__default["default"]({
...
function getExtendingRules(selectorIdMatch, extendAtRule) {
  // extending rules
  const extendingRules = []; // for each rule found from root of the extend at-rule with a matching selector identifier
  ...
  extendAtRule.root().walkRules(selectorIdMatch, matchingRule => {
    ...
    const nestingSelectors = matchingRule.selectors.filter(selector => selectorIdMatch.test(selector)).map(selector => selector.replace(selectorIdMatch, '$1&$3')).join(','); // matching rule’s cloned nodes

    const nestingNodes = matchingRule.clone().nodes; // clone the matching rule as a nested rule
...
function getSelectorIdMatch(selectorIds, postcss) {
  ...
  const escapedSelectorIds = postcss.list.comma(selectorIds).map(selectorId => selectorId.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|'); // selector unattached to an existing selector
